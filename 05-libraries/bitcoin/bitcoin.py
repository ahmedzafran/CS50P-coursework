...
Note that the requests module comes with quite a few methods,
 per requests.readthedocs.io/en/latest, among which are get,
 per requests.readthedocs.io/en/latest/user/quickstart.html#make-a-request,
 and json, per requests.readthedocs.io/en/latest/user/quickstart.html#json-response-content. You can install it with:
...
def main():
    ...
    data = r.json()
    bit_rate = data['bpi']['USD']['rate_float']
    ...
    if len(sys.argv) == 1:
        ...
        try:
            ...
            cus_amount = amount*bit_rate
            print(f"${cus_amount:,.4f}")
        except:
