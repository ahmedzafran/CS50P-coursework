...
Harvard traditionally receive their very own I took CS50 t-shirt.
...
If you’d like to run your program on a photo of yourself,
 ...
 into the same folder as shirt.py. No need to submit any photos with your code.
  But, if you would like, you’re welcome (but not expected)
  to share a photo of yourself wearing your virtual shirt in any of CS50’s communities!
"""
import sys
...
def main():
    if 2 < len(sys.argv) < 4:
        ...
        ext1, ext2 = path.splitext(file_name), path.splitext(output_name)
        match ext2[1]:
            case ".jpg" | ".jpeg" | ".png":
                if ext1[1] == ext2[1]:
                    try:
                        shirt_fit(file_name, output_name)
...
def shirt_fit(file_name, output_name):
    try:
