...
a variable for a user’s name might be called name, a variable for a user’s first name might be called firstName,
...
a variable in camel case and outputs the corresponding name in snake case. Assume that
the user’s input will indeed be in camel case.

"""
def main():
    enter = input("please enter a camelCase input: ")
    snake_case(enter)
...
def snake_case(sentence):
    for letter in sentence:
        if letter.isupper():
            letter_new = letter.replace(letter, "_" + letter.lower())
            sentence = sentence.replace(letter, letter_new)
    print(sentence)
    return sentence
